...
public class eventDbUpload
{
  protected Logger logger = Logger.getLogger(eventDbUpload.class);
  ...
  private String insertPlannedOrdersIntoDB(Connection connection, DBUploadService.Argument argDBUpload) throws ParseException, SQLException
  {
    ...
    try
    {
      ...
      for (Order ord : listOrder){
    	  if (ord.getParameterBy("script").getValueAsString().equalsIgnoreCase("ReassignManually"))
      ...
      for (Order o1 : PlanOrders)
      {
        ...
        if (order.getState().equals(PlanningState.PLANNED))
        {
         ...
         String marshal_name = order.getParameterBy("Marshal_name").getValueAsString();
          ...
          String remarks = "Manually Assigned";
      ...
      /*    if(order.getType().contains("KKCT") || order.getType().contains("CB")){
          ...
          script.setValue("ManuallyAssigned");  
