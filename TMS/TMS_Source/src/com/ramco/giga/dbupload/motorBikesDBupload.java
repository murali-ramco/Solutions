...
public class motorBikesDBupload
{
  protected Logger logger = Logger.getLogger(motorBikesDBupload.class);
  ...
  private String insertPlannedOrdersIntoDB(Connection connection, DBUploadService.Argument argDBUpload) throws ParseException, SQLException
  {
    ...
    try
    {
      ...
      for (Order ord : listOrder){
    	  if (ord.getType().equalsIgnoreCase("MB"))
      ...
      for (Order o1 : PlanOrders)
      {
        ...
        if (order.getState().equals(PlanningState.PLANNED))
        {
    ...
    //      if (!order.getType().equalsIgnoreCase("IndividualOrders")){
          ...
          String LocationId;
          for (StateValue stringValue : stateVariable.getValues()) {
            ArrayList updateTruckInfo = new ArrayList();
            LocationId = (String)stringValue.getValue();
            this.logger.info("locationId :" + LocationId);
          ...
          truckResUpdatePS.executeBatch();
            ...
            /*if(order.getType().equalsIgnoreCase("IndividualOrders")){
            	remarks = "No trucks are available to fulfill the order.";
            	evaluatedTrucks = "";
     ...
     /*   else if((order.getState().equals(PlanningState.CREATED))){
        	...
        	this.logger.info("Unplanned");

            String remarks = (Integer)order.getParameterBy("plan_ou").getValue();
            logger.info("ou : "+ou);
